...
function updateTemplateBuilder() {
  ...
  // 2. Add table_group to FIELD_TYPES
...
`;
  ...
  const mapStart = code.indexOf('<div className="flex flex-col gap-2 mb-3">');
  const mapEndStr = '</section>';
  const mapEnd = code.indexOf(mapEndStr, mapStart);
  ...
  // Wait, I need to match the section correctly.
  const oldMapRegex = /<div className="flex flex-col gap-2 mb-3">[\s\S]*?<\/section>/;
  code = code.replace(oldMapRegex, mapCode);

  // 6. Replace Map logic in LivePreview
  ...
  console.log("TemplateBuilder rewritten perfectly.");
}
