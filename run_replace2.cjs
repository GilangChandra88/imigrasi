...
code = code.replace(/<div className="grid grid-cols-\[110px_1fr\]/g, '<div className="spd-grid grid grid-cols-[110px_1fr]');
code = code.replace(/<div className="pt-1/g, '<div className="spd-label pt-1');
code = code.replace(/<span className="w-5 shrink-0 pt-1 text-center">:</g, '<span className="spd-colon w-5 shrink-0 pt-1 text-center">:');
...
const oldBlock = `<div className="flex-1 flex flex-col gap-3 text-justify text-[15px] leading-snug">
                {(template.fields || []).map(renderFieldDoc)}
              </div>`;

code = code.replace(oldBlock, newRenderBlock);
